...
# 설정
TEST_RESULTS_DIR = "test_results_dynamic"  # 동적 손익비 시뮬 결과만 사용
PATTERN_LOG_DIR = "pattern_data_log_dynamic"  # ⭐ 동적 손익비 패턴 로그!
...
def extract_features_from_pattern(pattern_stages: Dict, signal_info: Dict) -> Dict:
    ...
    # ⭐ 동적 손익비 목표값 추가 (ML 모델이 목표 손익비를 알 수 있도록)
    # 패턴 로그 형식 → extract_pattern_from_debug_info 형식으로 변환
    uptrend_raw = pattern_stages.get('1_uptrend', {})
    decline_raw = pattern_stages.get('2_decline', {})
    support_raw = pattern_stages.get('3_support', {})

    debug_info = {
        'uptrend': {
            'max_volume': uptrend_raw.get('max_volume', 0),
            'avg_volume': uptrend_raw.get('volume_avg', 0)  # ← volume_avg로 읽기
        },
        'decline': {
            'avg_volume': decline_raw.get('avg_volume', 0)
        },
        'support': {
            'avg_volume': support_raw.get('avg_volume', 0)
        }
    }
    ...
    features['target_stop_loss'] = ratios['stop_loss']
    features['target_take_profit'] = ratios['take_profit']
